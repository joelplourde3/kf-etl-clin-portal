...
case class PollingConfig(interval: Duration)

case class AWSConfig(endpoint: String, pathStyleAccess: Boolean, bucketName: String)

case class KeycloakConfig(cookie: String)

case class FhirConfig(baseUrl: String, schemaPath: String, resources: List[FhirRequest])

case class FhirRequest(`type`: String, schema: String, tag: String, total: Option[String], filter: Option[String], count: Option[Int])

case class Config(pollingConfig: PollingConfig,
                  awsConfig: AWSConfig,
